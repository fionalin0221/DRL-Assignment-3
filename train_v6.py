...
class Agent():
    ...
    def learn(self):
        ...
        max_q_action = self.policy_model(next_state_batch).max(1)[1]
        next_state_action_value = self.target_model(next_state_batch).gather(1, max_q_action.unsqueeze(1)).squeeze().detach()
