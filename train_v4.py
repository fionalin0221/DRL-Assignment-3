...
class ICM(nn.Module):
    def __init__(self, in_channels=4, num_actions=12):
        ...
        # Forward model: feature + action → 512 → decoder
        self.pred_module1 = nn.Linear(512 + num_actions, 512)

        # Decoder: 512 → 7x7x64 → upsample to 84x84x4
        self.decoder_fc = nn.Linear(512, 7 * 7 * 64)
        self.deconv1 = nn.ConvTranspose2d(64, 64, kernel_size=3, stride=1)      # 7x7 → 9x9
        self.deconv2 = nn.ConvTranspose2d(64, 32, kernel_size=4, stride=2)      # 9x9 → 20x20
        self.deconv3 = nn.ConvTranspose2d(32, in_channels, kernel_size=8, stride=4, output_padding=0)  # 20x20 → 84x84
    ...
    def pred(self, feature_s, a_vec):
        x = torch.cat([feature_s, a_vec.float()], dim=-1)
        x = F.relu(self.pred_module1(x))
    ...
    def get_full(self, s, s_next, a_vec):
        feature_s = self.get_feature(s)
        pred_s_next = self.pred(feature_s, a_vec)
        return pred_s_next, s_next  # s_next is the target
...
class Agent():
    ...
    def learn(self):
        ...
        # with torch.no_grad():
        #     r_icm = 1e5 * F.mse_loss(pred_s_next, feature_s_next, reduction='none').mean(dim=1) + F.cross_entropy(pred_a_vec, action_batch, reduction='none')

        # mse_loss = 1e5 * F.mse_loss(pred_s_next, feature_s_next.detach())
        # cross_entropy_loss = F.cross_entropy(pred_a_vec, action_batch)
        # loss_icm = mse_loss + cross_entropy_loss

        ### Training ICM 
        next_state_pred, next_state_target = self.icm_model.get_full(state_batch, next_state_batch, one_hot_action_batch)

        loss_icm_batch = F.mse_loss(next_state_pred, next_state_target.detach(), reduction='none').mean(dim=[1, 2, 3])
        loss_icm = loss_icm_batch.mean()
        r_icm = loss_icm.detach()
        ...
        expected_state_action_values = reward_batch + self.gamma * (1-done_batch.float()) * next_state_action_value + self.beta * r_icm
